...
const BlockPayment = (props) => {
    console.log(props.listPayment.paymentList)
    console.log(props.listPayment)
    let allPayment=props.listPayment.paymentList.map(nav=>(
        <NavLink className={classes.navLink} to={'/myPayment/account'}>
            <div className={'row mb-3 mt-3 mr-0 ml-0 ' + ' ' + classes.listOrder}>
                ...
                <div className={'col-lg-2 mb-lg-0 mb-3 col-12 '} style={{borderLeft: '2px solid' + ' ' + 'red'}}>
                    <span className={classes.textHeader}>Счет от {nav.date}</span>
                    <p className={classes.text}>Требуется оплата</p>
                </div>
                <div className={'col-lg-3 mb-lg-0 mb-3 col-12' + ' ' + classes.cityIn}>
                    <span className={classes.textHeader}>{nav.order_number}</span>
                    <p className={classes.text}>Номер счета</p>
                </div>
                ...
                <div className={'col-lg-1 mb-lg-0 mb-2 col-6'}>
                    <span className={classes.textHeader} style={{color: '#28a4eb'}}>{nav.order_count}</span>
                    <p className={classes.text}>Заявки</p>
                </div>
                <div className={'col-lg-1 col-6 pl-lg-0 pr-lg-0'}>
                    <span className={classes.textHeader}>{nav.amount} ₽</span>
    ...
    return allPayment.length===0
        ? <div className={'row mt-5'}>
            <div className={'col-12 mt-2 text-center'}>
                <span className={classes.textStyle}>Нет созданных счетов.</span>
            </div>
        </div>
        : allPayment
